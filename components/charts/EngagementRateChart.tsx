...
export default function EngagementRateChart({ posts }: { posts: Post[] }) {
  const data = posts.map((post) => ({
    name: shortenLabel(post.title),
    "Engagement Rate": Number(post.engagementRate.toFixed(2)),
  }));

  return (
    <ChartCard title="Engagement Rate by Post">
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} margin={{ top: 8, right: 8, left: 24, bottom: 24 }}>
          ...
          />
          <Bar dataKey="Engagement Rate" fill={CHART_COLORS.mustard} radius={[6, 6, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
  );
